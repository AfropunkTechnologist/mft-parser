...
fn process_file(filename: &str,options: ArgMatches) -> bool {
    ...
    let mut mft_handler = match MftHandler::new(filename) {
        ...
        Err(error) => {
            warn!("Couldn't parse file: {} [error: {}]", filename, error);
            return false;
        }
    };
                    ...
                    Some(ref j_expr) => {
                        ...
                        if expr_as_bool {
                            match result.as_boolean() {
                                ...
                                None => {
                                    panic!("Query expression isn't a bool expression!");
                                }
                            }
                        } else {
                            println!("{}",result)
...
fn main() {
    ...
    let options = App::new("RustyMft")
        .version("0.1.0")
        .author("Afropunk Technologist <https://github.com/AfropunkTechnologist/mft-parser>")
        .about("Parse $MFT.")
        .arg(source_arg)
        .arg(jmes_arg)
        .arg(bool_arg)
    ...
    if is_directory(source) {
        panic!("Directory source isn't implemented yet.");
    } else {
